...
vec4 blinnPhong() {
    ...
    for (int i = 0; i < MAX_LIGHTS; i++) {
        ...
        componentDifosa += atten*lights[i].Id*material.Kd*max(0.0f, dot(L, normalize(normal)));
        componentEspecular += atten*lights[i].Is*material.Ks*pow(max(dot(normalize(normal), H), 0.0f), material.shininess);
    }
